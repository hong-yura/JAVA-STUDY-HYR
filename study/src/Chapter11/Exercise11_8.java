...
import java.util.*;
/*
 * [11-8] 문제 11-7의 Student클래스에 총점(total)과 전교등수(schoolRank)를 저장하기 위한 인스턴스변수를 추가하였다.
 * Student클래스의 기본정렬을 이름이 아닌 총점(total)을 기준으로 한 내림차순으로 변경한 다음,
...
class Student8 implements Comparable {
	...
	public int compareTo(Object o) { 
		...
		if(o instanceof Student8) {
			Student8 stuTotal = (Student8)o;
			return (this.total - stuTotal.total) * (-1);
		} else {
			return -1;
		}
	}
	...
	public static void calculateSchoolRank(List list) {
		...
		/*
		 	1. 반복문을 이용해서 list에 저장된 Student객체를 하나씩 읽는다.
		 		1.1 총점(total)이 이전총점(prevTotal)과 같으면, 이전등수(prevRank)를 등수(schoolRank)로 한다.
